---
title: "North Sea International Bottom Trawl Survey (IBTS) - catch at sea"
output: html_notebook
---

# At sea analysis of catch data from the Barents Sea Ecosystem survey 2019
Developed during leg 1 of IBTS Q1 2020 (6-19th February)
...
ggsave("./analyses/L_W_plots/LW_plot_IBTS2020Q1.png")
```
...
```{r map catches}
bs<- map_data("worldHires", c("Norway", "Sweden", "UK", "Denmark", "Germany")) 
...
p <- p + geom_point(data=track, size=0.05, colour="lightblue", aes(x=lon, y=lat)) +  geom_polygon(data =bs, colour="gray35", fill="gray85", aes(x=long, y=lat, group=group))   +  coord_cartesian(xlim = c(15, 36), ylim=c(69, 74)) + theme_bw() + geom_jitter(width = 0.09, height = 0.09, aes(colour=sttype, shape=sttype))   +ggtitle("Survey track and stations, IBTS 2020 Q1")
p
