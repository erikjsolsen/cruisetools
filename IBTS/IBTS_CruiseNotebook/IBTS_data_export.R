...
samples <- read.csv2("./Data/Biotic/catchsample.csv", header=TRUE, dec=".")
individual <- read.table("./Data/Biotic/individual.txt", header=TRUE, sep="\t", dec=".")
station <- read.table("./Data/Biotic/fishstation.txt", header=TRUE, sep="\t", dec=".")
...
catchtable <- data.frame(ship="GOS", serial=st.list[2:length(st.list)], rect="NN", herr=0, cod=0, had=0, whi=0, Npout=0, sprat=0, mack=0, MIK=0, MIKeyM=0)
...
write.csv2(catchtable, "./analyses/catchtable.csv")
